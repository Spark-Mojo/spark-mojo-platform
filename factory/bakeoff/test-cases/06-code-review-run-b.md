# Test 06: Adversarial Code Review — Run B (Scheduling Module)

**Task type:** Code review / bug detection
**Evaluates:** Can the model catch all 5 planted violations across a React JSX file and a Frappe DocType controller?

**Scoring is binary per violation: caught (1) or not caught (0).**
**This is Run B. The 5 violation categories are identical to Run A and Run C, but the code and manifestations are different.**
...
1. **[REACT-FRAPPE]** `fetch('http://localhost:8000/api/method/sm_scheduling.api.get_providers...')` in a `useEffect` — bypasses the abstraction layer
2. **[TYPESCRIPT]** `interface TimeSlot { ... }` declaration at the top of the .jsx file
3. **[HEX-COLOR]** `style={{ color: '#6B7280' }}` on a label element — hardcoded hex color
4. **[SM-PREFIX]** `frappe.db.exists("Appointment Type", ...)` — missing "SM " prefix; should be "SM Appointment Type"
5. **[N8N-BOUNDARY]** `smtplib.SMTP` used in `_send_confirmation_email` called from `after_insert` — direct email send instead of routing through n8n
...
|-----------|-----------|------------|
| [REACT-FRAPPE] fetch to localhost:8000/api/method/sm_scheduling.api.get_providers | | |
| [TYPESCRIPT] interface TimeSlot declaration in .jsx file | | |
| [HEX-COLOR] style={{ color: '#6B7280' }} on label element | | |
| [SM-PREFIX] frappe.db.exists("Appointment Type") — missing SM prefix | | |
| [N8N-BOUNDARY] smtplib.SMTP in _send_confirmation_email called from after_insert | | |

**Score: {caught}/5**
**Pass threshold: 4/5**

Note: Do not penalize for flagging other legitimate issues. Score only the 5 planted violations.
